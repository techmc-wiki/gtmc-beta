...
- **The opening paragraph carries a raised versal.** The first letter after the article H1 is set large in the *sans body face* — not the serif title face. A versal belongs to the text block it opens; a serif initial here would echo the serif H1 directly above it and read as a second, competing title. It is *raised* (sits on the baseline), not a sunk/floated drop cap — many articles open with a one-line intro, and a floated cap would collide with the block beneath. A small gap sits between the title rule and the opening paragraph so the versal opens the text rather than crowding the title. The incipit (first line) is set in small-caps. CJK openings keep the versal but opt out of small-caps, which has no meaning for Han characters.
